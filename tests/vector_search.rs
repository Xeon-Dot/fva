...
fn test_store() -> (Arc<dyn VectorStore>, Arc<dyn Embedder>) {
    ...
    let store = Arc::new(FlatVectorStore::open(
...
fn make_chunks() -> Vec<CodeChunk> {
    vec![
        make_chunk("auth:login", "login_user", "fn", "fn login_user(username: &str, password: &str) -> bool {\n    // authenticate user\n    true\n}", "src/auth.rs"),
        make_chunk("auth:logout", "logout_user", "fn", "fn logout_user(session: &str) {\n    // clear session\n}", "src/auth.rs"),
        make_chunk("auth:validate", "validate_token", "fn", "fn validate_token(token: &str) -> Result<User> {\n    // verify JWT token\n    Ok(User)\n}", "src/auth.rs"),
        make_chunk("db:query", "query_users", "fn", "fn query_users(conn: &Connection) -> Result<Vec<User>> {\n    conn.query(\"SELECT * FROM users\")\n}", "src/db.rs"),
        make_chunk("db:insert", "insert_user", "fn", "fn insert_user(conn: &Connection, user: &User) -> Result<()> {\n    conn.execute(\"INSERT INTO users VALUES\", user)\n}", "src/db.rs"),
        make_chunk("ui:render", "render_html", "fn", "fn render_html(template: &str, data: &Data) -> String {\n    template.render(&data)\n}", "src/ui.rs"),
        make_chunk("ui:format", "format_date", "fn", "fn format_date(dt: &DateTime) -> String {\n    dt.format(\"%Y-%m-%d\")\n}", "src/ui.rs"),
        make_chunk("sort:bubble", "bubble_sort", "fn", "fn bubble_sort(arr: &mut [i32]) {\n    for i in 0..arr.len() {\n        for j in 0..arr.len()-i-1 {\n            if arr[j] > arr[j+1] {\n                arr.swap(j, j+1);\n            }\n        }\n    }\n}", "src/sort.rs"),
        make_chunk("sort:quick", "quick_sort", "fn", "fn quick_sort(arr: &mut [i32]) {\n    if arr.len() <= 1 { return; }\n    let pivot = partition(arr);\n    quick_sort(&mut arr[..pivot]);\n    quick_sort(&mut arr[pivot+1..]);\n}", "src/sort.rs"),
        make_chunk("config:parse", "parse_config", "fn", "fn parse_config(path: &Path) -> Result<Config> {\n    let content = std::fs::read_to_string(path)?;\n    toml::from_str(&content)\n}", "src/config.rs"),
        make_chunk("http:handle", "handle_request", "fn", "fn handle_request(req: &Request) -> Response {\n    match req.method() {\n        Method::GET => handle_get(req),\n        Method::POST => handle_post(req),\n        _ => Response::not_found(),\n    }\n}", "src/http.rs"),
        make_chunk("http:parse", "parse_body", "fn", "fn parse_body(req: &Request) -> Result<Body> {\n    let bytes = req.body_bytes();\n    serde_json::from_slice(&bytes)\n}", "src/http.rs"),
        make_chunk("math:add", "add_numbers", "fn", "fn add_numbers(a: i32, b: i32) -> i32 { a + b }", "src/math.rs"),
    ]
}
...
fn test_embedding_quality_semantically_related() -> Result<()> {
    ...
    println!("  auth↔login: {:.4}, auth↔html: {:.4}, auth↔db: {:.4}", sim_auth_login, sim_auth_html, sim_auth_db);

    // The authentication embedder should find login more related than HTML rendering
    assert!(sim_auth_login > sim_auth_html,
    ...
    if sim_auth_db > sim_auth_html {
        println!("  ✓ auth-db ({:.4}) > auth-html ({:.4})", sim_auth_db, sim_auth_html);
    }
...
fn test_search_quality_relevant_results_on_top() -> Result<()> {
    ...
    for (i, hit) in results.iter().enumerate() {
        println!("  #{:<3} {:.4}  {} ({})", i+1, hit.score, hit.symbol_name, hit.relative_path);
    }

    // The top 3 results should include auth-related functions
    let top_names: Vec<&str> = results.iter().take(3).map(|h| h.symbol_name.as_str()).collect();
...
fn test_search_rejects_unrelated_code() -> Result<()> {
    ...
    let top5_names: Vec<&str> = results.iter().take(5).map(|h| h.symbol_name.as_str()).collect();
    println!("  Top 5: {:?}", top5_names);

    let has_sort = top5_names.iter().any(|n| *n == "bubble_sort" || *n == "quick_sort");
    assert!(has_sort,
...
fn test_parallel_search_performance() -> Result<()> {
    ...
    println!("  Indexing {} chunks for performance test", all_chunks.len());
    index_chunks(embedder.as_ref(), store.as_ref(), &all_chunks)?;

    let stats = store.stats();
    println!("  Vector store: {} vectors, {} dimensions", stats.total_vectors, stats.dimensions);
...
fn test_token_index_properly_built() -> Result<()> {
    ...
    assert!(found.contains(&"validate_token"),
...
fn test_chunk_by_id_o1() -> Result<()> {
    ...
    let vectors: Vec<Vec<f32>> = chunks.iter()
        .map(|c| embedder.embed_one(&format!("{} {} {}", c.language, c.symbol_kind, c.content)).unwrap())
        .collect();
